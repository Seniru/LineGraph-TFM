...
## class `Series`
Defines methods and fields to create and update data series which hold data that could be inserted into any `LineChart` instance.This class is to hold data and methods that could be display in a line chart.
...
**`Series.new(dx, dy, name, col)`**
 ...
 - Parameters
  - **dx** *(number[])*: 'x'`x` data
  - **dy** *(number[])*: 'y'`y` data
  - **name** *(string)*: NThe name of the Sseries 
  - **col** *(integnumber)*: The color of the sSeries. May specified using hexadecimal format
...
**`LineChart.new(id, x, y, w, has a hexadecimal value.
...
| :getDataLengthLineColor() | | `number`<br>The length of all data | Returns the length of the points of all series included in the instance |
| :getGraphColorine color. The default values is 0xFF6600 | Returns the line color  |
| :getAlpha() | | `number[]`<br>The graph background and the border color | Returns the background and the border color as a table. The values can be accessed using bgColor and borderColor indexes |
| :getAlpha() | | `number`<br>The alpha (transparency). Default is alpha (transparency). Default is 1 | Returns the alpha |
...
`range(from, to, step)` Returns a continuous list: Generates and returns a sequence of numbers from `from` to `to` with an interval of `step`
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTIxODc2NTddfQ==
-->